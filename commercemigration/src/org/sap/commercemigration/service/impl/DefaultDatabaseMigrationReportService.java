...
public class DefaultDatabaseMigrationReportService implements DatabaseMigrationReportService {
    ...
    private void populateConfiguration(MigrationReport migrationReport) {
        ...
        final Set<String> maskedProperties = Arrays.stream(config.getString(MIGRATION_REPORT_MASKED_PROPERTIES)
        ...
        while (keys.hasNext()) {
            ...
            configuration.put(prefixedKey, maskedProperties.contains(prefixedKey) ? MASKED_VALUE : stripJdbcPassword(subset.getString(key)));
        }
