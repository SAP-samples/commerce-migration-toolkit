...
public class OracleDataRepository extends AbstractDataRepository {
	...
	protected String buildOffsetBatchQuery(OffsetQueryDefinition queryDefinition, String... conditions) {
		...
		return String.format("select * " + " from ( " + " select /*+ first_rows(%s) */ " + "  t.*, " + "  row_number() "
				+ "  over (order by %s) rn " + " from %s t where %s) " + "where rn between %s and %s " + "order by rn",
				queryDefinition.getBatchSize(), orderBy, queryDefinition.getTable(), expandConditions(conditions),
				queryDefinition.getOffset() + 1, queryDefinition.getOffset() + queryDefinition.getBatchSize());
	}
	...
	protected String buildValueBatchQuery(SeekQueryDefinition queryDefinition, String... conditions) {
		return String.format(
				"select * " + " from ( " + " select /*+ first_rows(%s) */ " + "  t.*, " + "  row_number() "
						+ "  over (order by t.%s) rn " + " from %s t where %s) " + "where rn <= %s " + "order by rn",
	...
	protected String createAllTableNamesQuery() {
		return String.format("select distinct TABLE_NAME from ALL_TAB_COLUMNS where lower(OWNER) = lower('%s')",
	...
	protected String createUniqueColumnsQuery(String tableName) {
		return String.format("SELECT t2.\"COLUMN_NAME\"\n" + "FROM\n" + "(\n" + "  SELECT * FROM (\n"
				...
				+ "ON t1.\"INDEX_NAME\" = t2.\"INDEX_NAME\" AND t1.\"OWNER\" = t2.\"INDEX_OWNER\" AND t1.\"TABLE_NAME\" = t2.\"TABLE_NAME\"",
